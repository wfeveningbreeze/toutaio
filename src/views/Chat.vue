...
created() {
  ...
  // 建立连接的事件,调用 socket.on('connect', fn) 方法，可以监听到 socket 连接成功的事件：
socket.on('connect', () => console.log('connect: websocket 连接成功！'));
// 关闭连接的事件,调用 socket.on('disconnect', fn) 方法，可以监听到 socket 连接关闭的事件：
socket.on('disconnect', () => console.log('disconnect: websocket 连接关闭！'));
// 接收到消息的事件,调用 socket.on('message', fn) 方法，即可监听到服务器发送到客户端的消息：
